# -*- coding: utf-8 -*-
"""
Created on Mon Sep  1 10:18:33 2025
...
EL = np.array([[1,2],
               ...
               [3,1]])
                        #boundary condition
                        #dirilichlit =-1, neumann =1
DorN = np.array([[-1,-1],
                 [1,-1],
                 [1,1]])
                        #forces condition
                        #unknown forces are assumed
Fu = np.array([[0,0],         
               [0,0],
               [0,-20]])
                        #displacment
...
PD = np.size(NL,1)                         #problem dimenion
NoN = np.size(NL,0)                        #no of nodes

ENL = np.zeros([NoN,6*PD])                 #extended node list

ENL[:,0:PD] = NL[:,:]                      #assigning the nodesto the ENL
ENL[:,PD:2*PD] = DorN[:,:]                 
